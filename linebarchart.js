...
interface Axis extends HTMLAttributes<SVGElement>{
  orient: any;
  axisType: any;
  scale: any;
  ticks: any;
...
/***************** Axis Start ****************/
class Axis extends PureComponent<Axis> {
  chart = createRef<any>();
...
const XYAxisLineChart: FC<Chart> = ({scales,margins,svgDimensions}) => {
const xAxisProps = {
      orient: 'Bottom',
...
const XYAxisBarChart: FC<Chart>  = ({scales,margins,svgDimensions}) => {
const xAxisProps = {
      orient: 'Bottom',
      translate: `translate(0,${svgDimensions.height - margins.bottom})`,
...
class Bar extends PureComponent<Bar>{
  chart = createRef<any>();
  
  constructor(props: Bar){
      super(props);
